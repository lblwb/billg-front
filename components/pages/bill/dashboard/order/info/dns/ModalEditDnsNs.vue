...
export default defineComponent({
    ...
    methods: {
        ...
        modalDnsHidden() {
            this.$store.dispatch("billing/order/info/dns/rec/showDnsRecModal", {status: false});
            this.$store.dispatch("billing/order/info/dns/rec/setDataDnsRecModal", {data: null});
        },
        parseSubDomain(itemRec) {
            try {
                if (itemRec.name === undefined && itemRec.zone_name === undefined) {
                    return ""
                }
    ...
    computed: {
        ...mapGetters({
            dnsRecEditModalShow: "billing/order/info/dns/rec/getDnsRecModalShow",
            dnsRecEditModalData: "billing/order/info/dns/rec/getDnsRecModalData"
        })
    },
    components: {
        ModalEdit,
